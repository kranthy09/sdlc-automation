...
All tests use:
  ...
  - The _model kwarg to inject a MagicMock — no real SentenceTransformer loaded.
"""
...
def _make_embedder(mock_model: MagicMock, registry: CollectorRegistry) -> object:
    ...
    return Embedder("BAAI/bge-large-en-v1.5", registry=registry, _model=mock_model)
...
def test_embed_returns_float_vector_of_correct_dimension() -> None:
    """embed() converts the model's numpy array into a list[float] of length 1024."""
    ...
    mock_model.encode.assert_called_once_with("AP invoice matching")
...
def test_embed_batch_returns_one_vector_per_text() -> None:
    """embed_batch() returns N vectors, one per input text, each 1024-dim."""
    mock_model = MagicMock()
    mock_model.encode.return_value = np.zeros((3, 1024), dtype=np.float32)
