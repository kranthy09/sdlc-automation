...
## Post-Phase 6 UI Fixes (2026-03-31)

### PARTIAL_FIT Configuration Steps visibility — `2670faa`

**Bug:** `FeatureReportCard` and `AtomDetailCard` silently hid the Configuration Steps section when the LLM returned `null` for both `configuration_steps` and `config_steps`. `ReviewCard` (HITL view) already showed a placeholder — the results view did not.

**Fix:**
- `FeatureReportCard.tsx` — gates section on `finalClass === 'PARTIAL_FIT'` (not `configSteps.length > 0`); shows steps or italic "not generated" message
- `AtomDetailCard.tsx` — added `config_steps` free-text fallback; gates on `PARTIAL_FIT` classification; shows same placeholder message

**Fallback chain (consistent across all three views):**
1. `configuration_steps: string[]` (structured list from LLM) — rendered as `<ol>`
2. `config_steps: string` (free-text from LLM) — wrapped in `[…]`, rendered as `<ol>`
3. `null` / empty — italic "Configuration steps were not generated by the LLM for this requirement."

**Files changed:** `ui/src/components/features/FeatureReportCard.tsx`, `ui/src/components/results/AtomDetailCard.tsx`
